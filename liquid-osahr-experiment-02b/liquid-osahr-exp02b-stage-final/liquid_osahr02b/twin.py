"""Typed 6G/MEC OSAHR model used by closed-loop Liquid-OSAHR Experiment 02B."""
...
def build_schema()->Schema:
    return Schema(
        ...
        schema_id="liquid-osahr-02b-6g",version="1.0.0",
    )
...
def _route_hazard(policy:str)->Expr:
    ...
    # lets Experiment 02B test physical-twin fidelity under interventions.
...
def build_model(policy:str,scenario:Scenario,cfg:TwinConfig|None=None,bounds:HazardBounds|None=None)->tuple[Model,PersistentIndex]:
    ...
    model=Model(graph,BoundaryState({}),rules,parameters=params,memory=memory,model_id=f"liquid-osahr-02b-{policy}",version="1.0.0")
    return model,index
