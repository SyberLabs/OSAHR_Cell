...
models=['oracle','cfc','gru_dt','constant']; policies=['throughput','semantic']; regimes=['id','sparse']; n_pairs=3; reps=2
rows=[];total=len(models)*len(policies)*len(regimes)*n_pairs*reps;count=0;st=time.perf_counter()
checkpoint=ROOT/'artifacts/results/osahr_twin_runs.partial.csv';checkpoint.parent.mkdir(parents=True,exist_ok=True)
for regime in regimes:
    fast=[t for t in bundle.tests[regime] if t.profile=='fast'][:n_pairs]
    robust=[t for t in bundle.tests[regime] if t.profile=='robust'][:n_pairs]
        ...
        for mn in models:
            ...
            for policy in policies:
                for rep in range(reps):
                    m=run_twin(
                        hazard_model=mn,regime=regime,policy=policy,scenario=scenario,replicate=rep,
                        fast_times=ft.times,fast_rates=fr,robust_times=rt.times,robust_rates=rr,
                        root_seed=80219,cfg=TwinConfig(),
                        verify_incremental=(mn=='cfc' and regime=='id' and scenario==0 and policy=='semantic' and rep==0),
                    )
...
out=ROOT/'artifacts/results/osahr_twin_runs.csv';checkpoint.replace(out)
